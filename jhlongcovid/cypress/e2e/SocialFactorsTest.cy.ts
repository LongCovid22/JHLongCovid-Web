...
function SocialFactorsCustom(HealthInsurance: string, DifficultyOfPaying: string, WorkSituation: string) {
  ...
  if (32 >= skipQuestionUntil) {
    ...
    // "Options": ["Yes", "No"],
    // "Branching": null
    RadioOnlyParseAndNext(HealthInsurance);
  }
  if (33 >= skipQuestionUntil) {
    ...
    // ],
    // "Branching": null
    RadioOnlyParseAndNext(DifficultyOfPaying);
  }
  if (34 >= skipQuestionUntil) {
    ...
    // ],
    // "Branching": null
    RadioOnlyParseAndNext(WorkSituation);
  }
...
describe("Vaccinations Test", () => {
  ...
  it("Yes Health Insurance, Somewhat difficult financials, retired", () => {
    SocialFactorsCustom("Yes", "Somewhat difficult", "Retired"); //No Long Covid
  });
