...
function MedicationsCustom(medications: string, medicationsArray: string[]) {
  skipQuestionUntil = 0;
  Start();
  ...
  if (10 >= skipQuestionUntil) {
    ...
    //   }
    // }
    RadioOnlyParseAndNextWithSkipQuestion(medications, ["No", "Do not know"], 12);
  }
  if (11 >= skipQuestionUntil) {
    ...
    MultiChoiceParseAndNext(medicationsArray);
...
function HospitalizationsCustom(hospitalized: string) {
  skipQuestionUntil = 0;
  Start();
  ...
  //Yes/No. If no go to [1][4]. A ["choice", "choice"]
  if (3 >= skipQuestionUntil) {
    RadioOnlyParseAndNextWithSkipQuestion(hospitalized, ["No"], 5);
  }
...
describe("Hospitalizations Test", () => {
  beforeEach(() => {
    cy.visit("http://localhost:3001");
  });

  it("Yes", () => {
    HospitalizationsCustom("Yes"); //No Long Covid
  });

  it("No", () => {
    HospitalizationsCustom("No"); //No Long Covid
  });
})
describe("Medications Test", () => {
  beforeEach(() => {
    // Assuming your application is running at http://localhost:3001
    cy.visit("http://localhost:3001");
  });
  it("Yes", () => {
    MedicationsCustom("Yes", ["Antiviral pill", "Do not know", "Antibiotics,", "Oral steroids, such a"]); //No Long Covid
  });

  it("No", () => {
    MedicationsCustom("No", []); //No Long Covid
  });

  it("Do not know", () => {
    MedicationsCustom("Do not know", []); //No Long Covid
  });
